...
class BSTDict(DictAbstract):
    ...
    # get all the values of an attribute of Country class from the bst dictionary
    def getatt_tree(self, attribute):
      
      if self._root is not None:
        return self._getatt_tree(self._root, attribute)

  # helper function that appends the attributes of the values of the nodes (the values of each key is a class, so we are accessing the attributes/members of the class) into a list
    def _getatt_tree(self, node, attribute):
      list = []
      if node:
        list = self._getatt_tree(node.left, attribute)
        list.append(getattr(node.value, attribute))
        list.append(getattr(node.value, "country_name"))
        list = list + self._getatt_tree(node.right, attribute)
      return list
